...
__export(index_exports, {
  ...
  API_TOKEN_COOKIE_NAME: () => API_TOKEN_COOKIE_NAME,
  APP_WORKSPACE_DATA_SESSION_KEY: () => APP_WORKSPACE_DATA_SESSION_KEY,
  ApiError: () => ApiError,
  BASE_URL: () => BASE_URL,
  BREAKPOINTS: () => BREAKPOINTS,
  DEFAULT_DURATION: () => DEFAULT_DURATION,
  DEFAULT_FETCH_API_CONFIG: () => DEFAULT_FETCH_API_CONFIG,
  DEFAULT_ICON: () => DEFAULT_ICON,
  DEFAULT_UPLOADER_OPTIONS: () => DEFAULT_UPLOADER_OPTIONS,
  ...
  copyWithButtonFeedback: () => copyWithButtonFeedback,
  ...
  initClipboardHandlers: () => initClipboardHandlers,
  insertElementToDOM: () => insertElementToDOM,
...
import_marked.marked.use(highlightExtension);
var convertMarkdownToHtml = (markdownText) => {
  if (!markdownText) {
    return "";
  }
  const html = import_marked.marked.parse(markdownText, { async: false, gfm: true });
  return html;
};
...
var fetchAPI = async (url, options = {}) => {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    try {
      ...
      clearTimeout(timeoutId);
      lastError = error;
      if (import_axios.default.isCancel(error) && !controller.signal.aborted) {
        break;
      }
      if (attempt < retries) {
        continue;
  ...
  if (import_axios.default.isCancel(lastError) || lastError instanceof DOMException && lastError.name === "AbortError") {
    throw new ApiError(
      408,
      "Request Timeout",
